...
import (
	...
	"github.com/afonsocraposo/advent-of-code/internal/utils/algorithms"
	...
	"github.com/afonsocraposo/advent-of-code/internal/utils/slicess"
)
...
func Main() {
	...
	for part := 1; part <= 2; part++ {
		...
		for _, example := range examples {
			exampleLines, err := filereader.ReadDayExample(year, day, example)
			if err != nil {
				log.Fatalln(err)
			}
			expectedSolution, err := filereader.ReadDayExampleSolution(year, day, example, part)
			if err != nil {
				continue
			}
		...
		inputLines, err := filereader.ReadDayInput(year, day, 1)
		if err != nil {
			log.Fatalln(err)
		}
